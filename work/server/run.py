...
def main(ui_stream=None):
    ...
    if others:
        print(f"  (this PC is also reachable at: {', '.join(others)} -- if the"
              f" phone can't connect, run this with the right one as an argument)")
    ...
    def sso_bridge_loop():
        while True:
            try:
                ...
                print(f"[bridge] restarting after: {e}", flush=True)
                time.sleep(1)
