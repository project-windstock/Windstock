...
ITEM_INCENSE_ORDINARY = ITEM_INCENSE   # 401, the only incense this client knows
ITEM_TROY_DISK = ITEM_LURE             # 501, the only lure this client knows
...
TEMPLATES_TS = 1_474_400_000_000    # bumped 2026-09-13: game_master.bin went MISSING from
                                    ...
                                    # (was 1_474_300_000_000.)
                                    # bumped 2026-08-16: swapped our home-CONVERTED
...
# Bundled, read-only resources (assets/, game_master.bin, ...) live at the
# resource root. From source that is the server folder; under PyInstaller it is
# the unpacked bundle directory. See windstock.config.paths.
_HERE = paths.RESOURCE_ROOT
...
import re as _re
import s2sphere
from windstock.game import world as _world
...
try:
    from windstock.game import gamedata as _gd  # generated by tools/convert_gm.py
...
def build_fort_details_response(fort_id, lat, lng) -> bytes:
    ...
    from windstock.game import world as _w
...
def build_upgrade_response(uid) -> bytes:
    ...
    if _cfg.get("pokemon", "real_powerups", cast=bool):
        try:
            from windstock.game import leveling
            iv_a, iv_d, iv_s = _ivs(uid)
...
def _raid_drop(b, now_ms):
    ...
    from windstock.game import world
    from windstock.game import rpc as _rpc
...
# World Manager's editor:
#   [item_id, name, chance%, min, max]
# chance is a plain percentage (0-100); an entry is rolled once per spin and, when
# it hits, contributes a random min..max of that item. Entries with chance 0 never
# drop but stay in the editor so they can be switched on. The table below is the
# classic 2016 drop: balls/potions/revives always, berries and specials sometimes.
LOOT_TABLE = [
    [1,   "Poke Ball",     100, 1, 3],
    [2,   "Great Ball",     25, 1, 2],
    [3,   "Ultra Ball",     10, 1, 1],
...
def loot_table():
    """The live loot table: settings.json wins if present, else the built-in
    LOOT_TABLE. Rows are sanitised to (item_id, name, chance, min, max) so a
    hand-edited or half-saved file can never crash a spin."""
    rows = _cfg.get("pokestops", "loot_table") or []
    table = []
    for r in rows if isinstance(rows, list) else []:
        try:
            if not isinstance(r, (list, tuple)) or len(r) < 5:
                continue
            iid, name, chance, lo, hi = (r[0], str(r[1]), int(r[2]), int(r[3]), int(r[4]))
            if iid <= 0 or hi < 1 or lo < 1:
                continue
            table.append((int(iid), name, max(0, min(100, chance)), lo, max(lo, hi)))
        except (TypeError, ValueError):
            continue
    if table:
        return table
    return [(i, n, c, lo, hi) for i, n, c, lo, hi in LOOT_TABLE]


def _loot_roll(rnd):
    """The items one spin awards, from the live loot table.

    Kept the proven shape of the old hard-coded version: every spin guarantees at
    least LOOT_MIN_ITEMS items in total (topped up from the first always-drop row,
    i.e. Poke Balls), never exceeds LOOT_MAX_ITEMS (trimmed from the extras first,
    never dropping an award below one), and each rolled entry contributes a random
    min..max count. Only the ''always Potion + Revive'' hard-coding is gone -- what
    drops is now exactly what the table says."""
    _lo = _cfg.get("pokestops", "min_items_per_spin", cast=int)
    _hi = max(_lo, _cfg.get("pokestops", "max_items_per_spin", cast=int))
    min_items = max(LOOT_MIN_ITEMS, _lo)
    max_items = max(1, min(LOOT_MAX_ITEMS, _hi))
    awards = []
    for iid, _name, chance, lo, hi in loot_table():
        if chance <= 0 or rnd.random() * 100.0 >= chance:
            continue
        awards.append((iid, rnd.randint(lo, hi)))
    if not awards:                      # table was all-zero: a bare ball still spins
        awards = [(ITEM_POKE_BALL, 1)]
    total = sum(c for _i, c in awards)
    if total < min_items:
        # Top up the Poke Ball row so a spin never comes up empty-handed.
        have = next((c for i, c in awards if i == ITEM_POKE_BALL), 0)
        rest = [(i, c) for i, c in awards if i != ITEM_POKE_BALL]
        awards = [(ITEM_POKE_BALL, have + min_items - total)] + rest
    # Trim back to the maximum, taking from the extras first and never dropping
    # any award below one, so every rolled entry survives the cut.
    total = sum(c for _i, c in awards)
    for i in range(len(awards) - 1, -1, -1):
        if total <= max_items:
            break
        iid, cnt = awards[i]
        take = min(cnt - 1, total - max_items)
        if take > 0:
...
# Windstock windsock, a JPEG) is served at /fortimg/_default.png whenever
...
DEFAULT_FORT_FILE = paths.path("default_fort.png")


def default_fort_png():
    """Bytes of the photo every photo-less PokeStop/Gym shows. Built in: the
    Windstock windsock (a JPEG, despite the historical name)."""
    import base64
    try:
...
# Cached per (biome, allow_legendaries, rarity weights, biomes.json revision).
_BIOME_POOLS = {}


def _biome_module():
    """The biomes module (biomes.json + the OSM terrain), or None if unavailable."""
    try:
        from windstock.geo import biomes as _bio
        return _bio
    except Exception:
        return None
...
def _biome(lat, lng):
    """The biome name at a location (see biomes.py), or None if unavailable."""
    bio = _biome_module()
    if bio is None or lat is None or lng is None:
        return None
    try:
        return bio.biome_for(lat, lng, _cfg.get("spawns", "biome_size", cast=int))
    except Exception:
        return None


def _biome_boost(pid, boosts):
    """How much a biome favours one species: the strongest boost among its
    one-or-two types, capped per rarity tier.

    A biome shifts WHICH commons/uncommons dominate -- it must NOT turn a rare
    into a common. The cap keeps a trophy a trophy: a very_rare gets no boost at
    all, so Dratini stays a trophy by the water even though Water is favoured
    there.
    """
    mult = 1
    for t in pokemon_types(pid):
        ...
        if m > mult:
            mult = m
    return min(mult, _BIOME_CAP[_tier_of(pid)])
...
def _biome_pool(biome, allow_leg):
    ...
    key = (biome, allow_leg, tuple(sorted(w.items())),
           bio.generation() if bio else 0)
    pool = _BIOME_POOLS.get(key)
    if pool is not None:
        return pool
    boosts = bio.type_boosts(biome) if bio else {}
    pool = []
    for pid in range(1, 152):
        if pid in _LEGENDARY and not allow_leg:
            continue
        pool += [pid] * (w[_tier_of(pid)] * _biome_boost(pid, boosts))
    _BIOME_POOLS[key] = pool or _spawn_pool()
    return _BIOME_POOLS[key]


def _spawn_mode(cfg=None):
    """The live wild-spawn source: "biomes" (the default), "cpdata" or "classic"."""
    c = cfg or _event_cfg()
    mode = str(c.get("spawn_mode", cpdata.DEFAULT_MODE)).lower()
    return mode if mode in cpdata.MODES else cpdata.DEFAULT_MODE


def _cpdata_enabled(cfg=None):
    """True when cpdata.json drives wild spawns.

    That covers the default "biomes" mode (cpdata.json's vanilla rarity, then
    re-weighted by the OSM biome at the spawn's location) and the plain "cpdata"
    mode (no terrain flavour). The mode lives in the live event config
    (``spawn_mode``) so /default and the World Manager can switch tables without
    a restart. If cpdata.json is missing or empty we fall back to the built-in
    tiers rather than spawning nothing.
    """
    return (_spawn_mode(cfg) in (cpdata.BIOME_MODE, cpdata.SPAWN_MODE)
            and cpdata.available())


def _cpdata_multiplier(biome, allow_leg):
    """A ``pid -> multiplier`` for cpdata.pick_species, from one biome's boosts."""
    bio = _biome_module()
    boosts = bio.type_boosts(biome) if bio else {}
    ...
    def mult(pid):
        if pid in _LEGENDARY and not allow_leg:
            return 0.0
        return float(_biome_boost(pid, boosts))

    return mult


def _nest_pick(rnd, lat, lng, now):
    """The nest species for this region right now, or 0 when it isn't a nest (or
    this roll is one of the normal biome spawns a nest area still has)."""
    if not _cfg.get("spawns", "nests", cast=bool):
        return 0
    bio = _biome_module()
    if bio is None:
        return 0
    try:
        nest = bio.nest_species(
...
def _day_night(rnd, pid, lng, now, reroll):
    """Shy away from a species that is out at the wrong time of day, so nocturnal
    Pokemon (Zubat, ghosts, ...) really are a night thing and vice versa.
    `reroll` returns another candidate species."""
    bio = _biome_module()
    if bio is None or not _cfg.get("spawns", "day_night", cast=bool):
        return pid
    try:
        wrong = bio.DAY_SPECIES if bio.is_night(now, lng) else bio.NIGHT_SPECIES
    except Exception:
        return pid
    if pid in wrong and rnd.random() < 0.6:
        for _ in range(3):
            alt = reroll()
            if alt and alt not in wrong:
                return int(alt)
    return pid
...
def _pick_species(rnd, cfg=None, lat=None, lng=None):
    ...
    In the normal 'all' mode this follows cpdata.json's vanilla 2016 rarity
    table; in the default "biomes" mode the OpenStreetMap biome at (lat, lng)
    re-weights those same odds, so different areas favour different Pokemon the
    way 2016's biomes did (Water by the river, Bug in the woods, city trash
    downtown), with the 2016 nests and day/night behaviour on top. An event that
    forces a species/list overrides all of it (a Pikachu event is a Pikachu event
    everywhere)."""
    if _FORCE_POKEMON:
    ...
    geo_ok = (lat is not None and lng is not None
              and _cfg.get("spawns", "biomes", cast=bool))
    # DEFAULT: the vanilla cpdata.json rarity table (spawn_percent_chance). In the
    # "biomes" mode the real terrain decides which of those species dominate here.
    if _cpdata_enabled(c):
        biome = (_biome(lat, lng)
                 if (geo_ok and _spawn_mode(c) == cpdata.BIOME_MODE) else None)
        if biome:
            mult = _cpdata_multiplier(biome, allow_leg)
            pid = cpdata.pick_species(rnd, mult)
            if pid:
                try:
                    now = int(time.time() * 1000)
                    nest = _nest_pick(rnd, lat, lng, now)
                    if nest:
                        return nest
                    return _day_night(rnd, pid, lng, now,
                                      lambda: cpdata.pick_species(rnd, mult))
                except Exception:
                    return pid
        else:
            pid = cpdata.pick_species(rnd)        # no terrain here: vanilla odds
            if pid:
                return pid
    if geo_ok:
        # "classic": the built-in rarity tiers, re-weighted by the same biome.
        try:
            now = int(time.time() * 1000)
            nest = _nest_pick(rnd, lat, lng, now)
            if nest:
                return nest
            pool = _biome_pool(_biome(lat, lng) or "", allow_leg)
            pid = int(rnd.choice(pool))
            return _day_night(rnd, pid, lng, now, lambda: int(rnd.choice(pool)))
        except Exception:
...
def _pick_cp(rnd, cfg=None, pid=None):
    c = cfg or _event_cfg()
    # DEFAULT: 60% of wild spawns sit on the species' most common wild CP, within
    # +/-30 (the rest fall through to the normal range roll below).
    if pid and _cpdata_enabled(c):
        cp = cpdata.pick_cp(rnd, pid)
        if cp is not None:
            return cp
...
def _config_generation():
    ...
    gen = 0
    try:
        from windstock.config import events as _e, settings as _s, places as _p
...
def build_get_map_objects_response(cell_ids, lat, lng) -> bytes:
    ...
    from windstock.config import places as _places
    _pl = _places.get()
    ...
    try:
        if _cfg.get("pokestops", "use_osm", cast=bool):
            from windstock.geo import pois as _pois
...
def resolve_username(fields):
    ...
    from windstock.net.sso import username_from_token
