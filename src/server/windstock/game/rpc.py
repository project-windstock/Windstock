...
def _build_returns(reqs, username, log):
    ...
    for rtype, msg in reqs:
        if rtype == P.RT.GET_PLAYER:
            ...
            from windstock.game import world as _wj
...
def handle(method, path, query, headers, body, log):
    ...
    # clients (baked to e.g. windstock.playit.plus): redirecting them to pgorelease
    ...
    if not reqs:
        ...
        if _buy_id:
            try:
                from windstock.game import shop as _shop
                from windstock.game import world as _w
                _w.use(username)
