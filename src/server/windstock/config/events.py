...
HERE = paths.ensure()                           # <exe folder>/data
...
DEFAULTS = {
    ...
    # Which wild-spawn table to use in "all" mode:
    #   "biomes"  (the DEFAULT) cpdata.json's vanilla rarity + the most common
    #             wild CP values, re-weighted by the OpenStreetMap biome at your
    #             location (see biomes.json) -- Water by the river, Bug in the
    #             woods, city trash downtown.
    #   "cpdata"  cpdata.json alone, with no terrain flavour.
    #   "classic" the built-in biome/rarity tiers instead of cpdata.json.
    # (/default re-selects "biomes"; the World Manager has the same buttons.)
    "spawn_mode": "biomes",
    # Testing switch: force ONE biome everywhere instead of reading the real
    # OpenStreetMap terrain. "" = normal (auto). Set from the World Manager's
    # "Force biome" dropdown; every spawn, and the map readout, follows it.
    "force_biome": "",
    "min_cp": 100,
    "max_cp": 1200,
...
def _sanitize(c):
    ...
    if c.get("species_mode") not in ("all", "list", "single"):
        c["species_mode"] = "all"
    if c.get("spawn_mode") not in ("biomes", "cpdata", "classic"):
        c["spawn_mode"] = "biomes"
    # Lowercase + letters only; an unknown name simply means "auto" (biomes.py
    # checks it against the real biome list before using it).
    c["force_biome"] = "".join(
        ch for ch in str(c.get("force_biome") or "").strip().lower()
        if ch.isalpha() or ch == "_")[:24]
