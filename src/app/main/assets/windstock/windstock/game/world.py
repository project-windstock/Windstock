        ...
        if p is None:
            ...
            if os.path.exists(path):
                ...
                if not have_any and os.path.exists(SAVE_FILE) and p.load_from(SAVE_FILE):
                    ...
                    # (Adopted legacy saves keep their complete state above.) This
                    # read must NOT be guarded by a blanket except: it silently ate
                    # any failure and left TUTORIAL complete, so a brand-new player
                    # was dropped straight onto the map with no tutorial, no avatar
                    # customisation and no starter. Fail loudly instead -- a broken
                    # config read is a bug, not something to paper over.
