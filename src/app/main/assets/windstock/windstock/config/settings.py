...
SETTINGS_FILE = paths.path("settings.json")
...
DEFAULTS = {
    ...
    "pokestops": {
        ...
        # the row stays visible in the World Manager's loot editor). On a hit the
        # spin awards a random min..max of that item. Edit rows in the World
        # Manager (PokeStop Loot section) or right here -- both hot-reload. The
        # rows below are the classic 2016 drop; keep this list in sync with the
        # LOOT_TABLE fallback in protocol.py if you ever change the defaults here.
        "loot_table": [
            [1,   "Poke Ball",    100, 1, 3],
            [2,   "Great Ball",    25, 1, 2],
            [3,   "Ultra Ball",    10, 1, 1],
    ...
    "spawns": {
        ...
        "biome_size": 12,
        # Ask the free OpenStreetMap (Overpass) API for the real terrain the
        # first time anyone plays in an area, and cache the answer in
        # data/osm_biomes.json (see biomes.json). false = stay fully offline:
        # every region keeps its hash-chosen biome instead.
        "osm_online": True,
    ...
    "progression": {
        ...
        # through it. Needs a client whose login/first-run flow is intact. ON by
        # default: without it a new trainer skips the tutorial, the character
        # customisation and the first-Pokemon pick entirely -- the whole point of
        # a first launch. Turn it off only if onboarding misbehaves on your client.
        "run_tutorial": True,
    ...
    "server": {
        ...
        #   "windstock" = the Windstock window (status, activity, buttons)
...
_README = [
    ...
    "   osm_online ........... true = ask OpenStreetMap for the real terrain the",
    "                          first time you play in an area (cached afterwards),",
    "                          so biomes match the actual rivers/woods/downtown",
    ...
    "",
    "progression:  (what a brand-new trainer sees on first login)",
    "   run_tutorial ......... true = new accounts run the real 2016 onboarding:",
    "                          legal screen, avatar (character) customisation,",
    "                          name pick and the first starter Pokemon. Existing",
    "                          accounts are never sent back through it.",
    "   tutorial_skip_starter  true = skip the slow scripted starter catch and make",
    "                          the first catch a normal wild one instead.",
    "",
